...
f_IDAT <- function(kokapena = kokapena, fitxategia = fitxategia){
    ...
    df_pool <- l  |>
        ...
        rename(numToAttrib = rowname,
               var = X1,
               attrib = X2) |>
        mutate(pad = factor(pad),
               var = factor(var),
    ...
    df_aldaketak <- data.frame(revs,
        ...
        pivot_wider(names_from = 'zer',
                    values_from = 'balioak',
                    values_fill = NA) |>
        mutate(noz = f_epoc_ekarri(as.numeric(timestamp)),
               pad = pad,
    ...
    df_aldaketak <- df_aldaketak$landu |>
        ...
        data.frame() |>
        mutate(kok_lerru = ifelse(is.na(kok_lerru), 0, kok_lerru),
               # kok_lerrun_nun = ifelse(is.na(kok_lerrun_nun), 0, kok_lerrun_nun)) |> # Honek arazoak emoten ditu formatu aldaketa hutsian dabixenin
               kok_lerrun_nun = ifelse(is.na(kok_lerrun_nun), kok_aurreko_kar_kop, kok_lerrun_nun)) |> # Honek arazoak emoten ditu formatu aldaketa hutsian dabixenin
    ...
    df_aldaketak <- df_aldaketak |>
        ...
        mutate(aldi = timestamp - lag(timestamp),
               lerro_aldaketa = kok_lerru - lag(kok_lerru)) |>
        ungroup() |>
        relocate(pad, revs, changeset, author, name,
