...
f_idatzia <- function(objektua,
            ...
            mutate(
                etenak = ifelse(aldi>dm,
                                paste0('<', ifelse(round(aldi/1000, 1)<=60,
                                                   round(aldi/1000, 1),
                                                   paste0(round(aldi/1000, 1)%/%60, 'm.',
                                                          round(round(aldi/1000, 1)%%60))), 'seg>', ''),
                                ''),
    ...
    if(lerroko_jauziak){
        ...
        df <- df |>
            group_by(author) |>
            arrange(author, revs) |>
            mutate(gehi = kok_lerrun_nun + ifelse(azktex == '>', newLen, -(newLen)),
                   jauzi =  kok_lerrun_nun-lag(gehi)) |>
            ungroup() |>
            arrange(revs) |>
            mutate(jauziB = ifelse(jauzi != 0,
                   ...
                   jauzia = ifelse(seg == TRUE, '', paste0('[', abs(jauzi),
